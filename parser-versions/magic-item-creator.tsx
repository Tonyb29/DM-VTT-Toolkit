...
// ── Per-feature feat item (draggable to character sheet) ─────────────────
function buildFeatureItem(feature: MagicItemFeature, itemName: string): object {
  const id = makeId(itemName, feature.name, 'feat')
  ...
  const activationType = desc.includes('bonus action') ? 'bonus'
    : desc.includes('reaction') ? 'reaction'
    : feature.recharge ? 'action'
    : ''

  return {
    _id: id,
    name: `${feature.name}${rechargeLabel}`,
    type: 'feat',
    system: {
      description: { value: `<p>${feature.description}</p>` },
      type: { value: 'monster', subtype: '' },
      requirements: itemName,
      activation: { type: activationType, cost: activationType ? 1 : null, condition: '' },
      uses,
    },
    flags: {},
    effects: [],
...
function buildMacro(item: any, actorName?: string, features: MagicItemFeature[] = []): string {
  const json = JSON.stringify(item, null, 2)
  const n = item.name
  const hasFeatures = features.length > 0
  const featureItems = features.map(f => buildFeatureItem(f, n))
  const featureDataJson = JSON.stringify(featureItems.map(({ _id, ...rest }: any) => rest), null, 2)
  ...
  if (actorName?.trim()) {
    ...
    return `// Magic Item: ${n}
// Add to actor: ${a}
// Generated by DM VTT Toolkit — dmtoolkit.org

(async () => {
  const itemData = ${json};
${hasFeatures ? `  const featureData = ${featureDataJson};` : ''}

  // ── Step 1: Add weapon to actor ──────────────────────────────────────────
  const actor = game.actors.getName("${a}");
  if (!actor) { ui.notifications.error("Actor not found: ${a}"); return; }
...
${hasFeatures ? `
  // ── Step 2: Create feature items in Items sidebar (drag to other sheets) ─
  const rootFolder = game.folders.getName("Magic Items") ??
    await Folder.create({ name: "Magic Items", type: "Item" });
  const itemFolder = game.folders.find(f => f.name === itemData.name && f.folder?.id === rootFolder.id) ??
    await Folder.create({ name: itemData.name, type: "Item", folder: rootFolder.id });
  const featFolder = game.folders.find(f => f.name === "Features" && f.folder?.id === itemFolder.id) ??
    await Folder.create({ name: "Features", type: "Item", folder: itemFolder.id });
  if (!featFolder) { ui.notifications.error("${n}: Features folder creation failed."); return; }
  await Item.create(featureData.map(f => ({ ...f, folder: featFolder.id })));
  ui.notifications.info(\`${n}: ${featCount} feature items created in Items sidebar\`);` : ''}
})();`
  }

  return `// Magic Item: ${n}
// Folder structure: Magic Items → ${n} → Features
// Generated by DM VTT Toolkit — dmtoolkit.org
...
${hasFeatures ? `  const featureData = ${featureDataJson};` : ''}
  ...
  // ── Step 2: Create the weapon item ───────────────────────────────────────
  const existing = game.items.getName(itemData.name);
  if (existing) {
    await existing.update({ system: itemData.system });
  } else {
    await Item.create({ ...itemData, folder: itemFolder.id });
  }
${hasFeatures ? `
  // ── Step 3: Create feature items ─────────────────────────────────────────
  const featFolder = game.folders.find(f => f.name === "Features" && f.folder?.id === itemFolder.id) ??
    await Folder.create({ name: "Features", type: "Item", folder: itemFolder.id });
  if (!featFolder) { ui.notifications.error("${n}: Features folder creation failed."); return; }
  await Item.create(featureData.map(f => ({ ...f, folder: featFolder.id })));
  ui.notifications.info(\`${n}: weapon + ${featCount} feature items ready — drag features from Items sidebar onto character sheets\`);` : `
...
export default function MagicItemCreator() {
  ...
  const [currentFeatures, setCurrentFeatures] = useState<MagicItemFeature[]>([])
  ...
  function finishItem(spec: MagicItemSpec) {
    ...
    const json = JSON.stringify(item, null, 2)
    const macro = buildMacro(item, actorName, features)
    setItemJson(json)
    setMacroText(macro)
  ...
  function updateMacro(json: string, name: string) {
    try {
      ...
      setMacroText(buildMacro(item, actorName, currentFeatures))
    } catch { /* ignore */ }
  }
  ...
  async function handleParseText() {
    ...
    try {
      ...
      setRawSpec(raw)
      console.log('[MagicItem] Raw Claude spec:', raw)
      const spec: MagicItemSpec = JSON.parse(raw.replace(/^```json?\s*/i, '').replace(/```\s*$/, '').trim())
      console.log('[MagicItem] Parsed spec — features:', spec.features)
      finishItem(spec)
    } catch (e: any) {
