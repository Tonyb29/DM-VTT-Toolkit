...
import { Download, Copy, AlertCircle, CheckCircle, Info, FileJson, Zap, Edit2, Save, X, TestTube, BarChart3, Shield } from 'lucide-react';

export default function Phase5StatsAbilitiesStable() {
  ...
  const parseStatBlock = (text) => {
    ...
    try {
      ...
      // Skills - Enhanced to parse bonuses properly
      stats.total++;
      ...
      stats.fields.push({ name: 'skills', value: skillsText, method: skillMatch ? 'exact' : 'default', conf: skillMatch ? 1.0 : 0 });

      // Parse individual skill bonuses
      const skillMap = {
      ...
      // Initialize all skills with base values
      Object.values(skillMap).forEach(shortName => {
        skills[shortName] = {
          value: 0,
          ability: skillAbilityMap[shortName],
      ...
      if (skillsText) {
        ...
        skillEntries.forEach(entry => {
          // Match "Stealth +6" or "Animal Handling +3" etc
  ...
  const startEdit = (fieldName) => {
    ...
    else if (fieldName === 'languages') setEditValue(output?.system?.traits?.languages?.value?.join(', ') || '');
    else if (fieldName === 'abilities') {
      const abs = output?.system?.abilities;
      setEditValue(`STR ${abs?.str?.value} DEX ${abs?.dex?.value} CON ${abs?.con?.value} INT ${abs?.int?.value} WIS ${abs?.wis?.value} CHA ${abs?.cha?.value}`);
    }
  };
  ...
  const saveEdit = () => {
    ...
    if (editField === 'name') {
      ...
      updated.system.traits.size = editValue.toLowerCase();
    }
      ...
      // Parse "STR 10 DEX 14 CON 12 INT 10 WIS 8 CHA 8" format
      const match = editValue.match(/STR\s+(\d+)\s+DEX\s+(\d+)\s+CON\s+(\d+)\s+INT\s+(\d+)\s+WIS\s+(\d+)\s+CHA\s+(\d+)/i);
      if (match) {
        updated.system.abilities.str.value = parseInt(match[1]);
        updated.system.abilities.dex.value = parseInt(match[2]);
        updated.system.abilities.con.value = parseInt(match[3]);
        updated.system.abilities.int.value = parseInt(match[4]);
        updated.system.abilities.wis.value = parseInt(match[5]);
        updated.system.abilities.cha.value = parseInt(match[6]);
      }
    }
        ...
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-2">
            ...
            <span className="bg-green-600 text-white px-3 py-1 rounded-full text-xs font-bold flex items-center gap-1">
              <Shield size={14} /> STABLE v1.0
            </span>
          </div>
          <p className="text-purple-200 font-semibold">Phase 5: Stats & Abilities Parser - Production Ready</p>
          <p className="text-sm text-purple-300">Handles Name, Size, Type, Alignment, AC, HP, Abilities (4 formats), CR, Skills, Senses, Languages</p>
        </div>
              ...
              <>
                ...
                <div className="bg-green-900/20 border border-green-500/30 rounded-lg p-4">
                  <h3 className="text-green-400 font-semibold text-sm mb-2 flex items-center gap-2">
                    <Shield size={16} /> STABLE v1.0 - Stats & Abilities Parser
                  </h3>
                  <div className="text-xs text-slate-300 space-y-1">
                    <p><strong className="text-white">✅ Parses:</strong> Name, Size, Type, Alignment, AC, HP, Abilities, CR, Skills, Senses, Languages</p>
                    <p><strong className="text-white">✅ Ability Formats:</strong> Clean numbers, with modifiers in parens, without parens, multiline</p>
                    <p><strong className="text-white">✅ Alignment:</strong> After size/type with comma, explicit label, or keyword search</p>
                    <p><strong className="text-white">✅ CR Protection:</strong> Won't confuse AC with CR</p>
                    <p><strong className="text-white">✅ Ready for:</strong> Phase 6 - Actions, Traits, Reactions, Legendary Actions</p>
                  </div>
                </div>
              </>
            ) : (
