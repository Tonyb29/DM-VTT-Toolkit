...
export default function StatBlockParser() {
  ...
  const parseStatBlock = (text) => {
    ...
    try {
      ...
      const speedDisplay = `${speeds.walk} ft.${speeds.fly ? `, fly ${speeds.fly} ft.` : ''}${speeds.climb ? `, climb ${speeds.climb} ft.` : ''}${speeds.swim ? `, swim ${speeds.swim} ft.` : ''}${speeds.burrow ? `, burrow ${speeds.burrow} ft.` : ''}`;
      stats.fields.push({ name: 'speed', value: speedDisplay, method: speedMatch ? 'exact' : 'default' });

      // ABILITIES - Enhanced for D&D Beyond table format
      stats.total++;
      let abilities = { str: 10, dex: 10, con: 10, int: 10, wis: 10, cha: 10 };
      
      // Try standard format first: STR 10 DEX 14 CON 12...
      let abMatch = text.match(/STR\s+(\d+)\s+DEX\s+(\d+)\s+CON\s+(\d+)\s+INT\s+(\d+)\s+WIS\s+(\d+)\s+CHA\s+(\d+)/i);
      
      // Try with modifiers: STR 10 (+0) DEX 14 (+2)...
      if (!abMatch) abMatch = text.match(/STR\s+(\d+)\s*\([+-]\d+\)\s+DEX\s+(\d+)\s*\([+-]\d+\)\s+CON\s+(\d+)\s*\([+-]\d+\)\s+INT\s+(\d+)\s*\([+-]\d+\)\s+WIS\s+(\d+)\s*\([+-]\d+\)\s+CHA\s+(\d+)\s*\([+-]\d+\)/i);
      
      // Try D&D Beyond table format (single line): Str 10 +0 +0 Dex 16 +3 +5...
      if (!abMatch) {
        const beyondMatch = text.match(/Str\s+(\d+)\s+[+-]\d+\s+[+-]\d+\s+Dex\s+(\d+)\s+[+-]\d+\s+[+-]\d+\s+Con\s+(\d+)\s+[+-]\d+\s+[+-]\d+[\s\S]*?Int\s+(\d+)\s+[+-]\d+\s+[+-]\d+\s+Wis\s+(\d+)\s+[+-]\d+\s+[+-]\d+\s+Cha\s+(\d+)\s+[+-]\d+\s+[+-]\d+/i);
        if (beyondMatch) {
          abMatch = beyondMatch;
        }
      }
      
      // Try D&D Beyond split table format (NEW - handles two separate tables)
      if (!abMatch) {
        // Look for each ability individually with flexible whitespace (handles tabs and spaces)
        ...
        if (strMatch && dexMatch && conMatch && intMatch && wisMatch && chaMatch) {
          abilities = {
            ...
            con: parseInt(conMatch[1]),
            int: parseInt(intMatch[1]),
            wis: parseInt(wisMatch[1]),
            cha: parseInt(chaMatch[1])
          };
          abMatch = true; // Set flag that we found abilities
        }
      }
      ...
      // CR & Proficiency Bonus - Enhanced for D&D Beyond format
      stats.total++;
      // Try with XP and PB: CR 4 (XP 1,100; PB +2)
      let crMatch = text.match(/CR\s+(\d+(?:\/\d+)?)\s*\([^)]*\)/i);
      // Try standard: Challenge 1/4
      ...
      // SENSES - Enhanced for Passive Perception
      stats.total++;
  ...
  const startEdit = (fieldName) => {
    ...
    else if (fieldName === 'initiative') setEditValue(output?.system?.attributes?.init?.bonus || '');
  ...
  const saveEdit = () => {
    ...
    else if (editField === 'initiative') updated.system.attributes.init.bonus = editValue;
        ...
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-2">
            ...
            <span className="bg-blue-600 text-white px-3 py-1 rounded-full text-xs font-bold flex items-center gap-1">
              <Shield size={14} /> v1.6 - D&D BEYOND COMPATIBLE
            </span>
          </div>
          <p className="text-purple-200">Now supports D&D Beyond table formats and initiative</p>
        </div>
          ...
          <div className="space-y-4">
            <div className="bg-slate-800 rounded-lg p-5 border border-purple-500/30">
              ...
              <textarea
                ...
                placeholder="Paste D&D 5e stat block here (supports D&D Beyond format)..."
              ...
              <>
                ...
                <div className="bg-blue-900/20 border border-blue-500/30 rounded-lg p-4">
                  <h3 className="text-blue-400 font-semibold mb-2 flex items-center gap-2">
                    <Shield size={16} /> v1.6 - D&D Beyond Compatible
                  </h3>
                  <div className="text-sm text-slate-300 space-y-1">
                    <p>✅ Supports D&D Beyond table format for abilities</p>
                    <p>✅ Parses Initiative bonus</p>
                    <p>✅ Enhanced CR parsing (handles XP and PB info)</p>
                    <p>✅ Flexible speed parsing (any order)</p>
                    <p>✅ All v1.5 features included</p>
                  </div>
                </div>
              </>
            ) : (
