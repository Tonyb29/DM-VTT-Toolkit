...
function buildMacro(spec: SubclassSpec, actorName: string): string {
  const subclassItem = buildSubclassItem(spec)
  ...
  const allItems = [subclassItem, ...featureItems]
  const itemsJson = JSON.stringify(allItems, null, 2)

  const actorBlock = actorName.trim() ? `
// Give to actor
...
ui.notifications.info("${spec.name} subclass items added to " + actor.name);` : `
// Create in Items sidebar
...
ui.notifications.info("${spec.name} subclass items created in Items sidebar.");`

  return `// ${spec.name} — Subclass Import Macro
// Generated by DM VTT Toolkit — dm-vtt-toolkit.halfasliceofchez.workers.dev
// Parent class: ${className} (identifier: "${spec.classIdentifier}")
